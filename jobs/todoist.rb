...
SCHEDULER.every '20m', :first_in => 0 do |job|
    ...
    item_url_string  = 'https://todoist.com/API/v8/sync?token=' + todoist_token + '&resource_types=["items", "projects"]&sync_token=\'*\''
    ...
    if response.code == "200"
        ...
        hash = Hash.new
        items.each do |st|
            dateString = st['due']['date']
            if dateString != nil
                date = Date.parse(dateString)
                difference = (date - Date.today).to_i
		formatDate = date.strftime("%m/%d")
                #puts projHash[st['project_id']].to_s
                hash[formatDate + " - " + projHash[st['project_id']].to_s + ": " +  + " " + st['content']] = difference
            end
        end
